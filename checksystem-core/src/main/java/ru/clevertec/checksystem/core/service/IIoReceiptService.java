...
public interface IIoReceiptService extends IEventEmitter<Object> {
    ...
    void write(Collection<Receipt> receipts, OutputStream os, FormatType formatType)
            throws IOException;

    void write(Collection<Receipt> receipts, File destinationFile, FormatType formatType)
            throws IOException;

    void serialize(Collection<Receipt> receipts, File destinationFile, StructureFormat structureFormat)
            throws IOException;

    void serialize(Collection<Receipt> receipts, OutputStream os, StructureFormat structureFormat)
            throws IOException;

    Collection<Receipt> deserialize(File sourceFile, StructureFormat structureFormat)
            throws IOException;

    Collection<Receipt> deserialize(InputStream is, StructureFormat structureFormat)
            throws IOException;
    ...
    void sendToEmail(String subject, Collection<Receipt> receipts, FormatType formatType, String... addresses)
