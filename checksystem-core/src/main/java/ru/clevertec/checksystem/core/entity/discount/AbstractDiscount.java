...
public abstract class AbstractDiscount<T extends AbstractDiscount<T>> extends BaseEntity {
    ...
    @JoinColumn(name = Constants.Entities.Mapping.JoinColumn.DEPENDENT_DISCOUNT_ID)
    private T dependentDiscount;

    @Column(name = Constants.Entities.Mapping.JoinColumn.DEPENDENT_DISCOUNT_ID, insertable = false, updatable = false)
