...
public class TextCheckLayout extends AbstractCheckLayout {

    private static final int MIN_SPACE_COUNT = 1;

    private int spacesNumber = 4;

    public TextCheckLayout() {
    }

    @Override
    public byte[] getLayoutData(Check check) {
        return createCheckText(check);
    }

    @Override
    public byte[] getAllLayoutData(Collection<Check> checks) {
        return checks.stream().map(check -> new String(createCheckText(check)))
                .reduce((a, b) -> a + '\n' + b)
                .map(String::getBytes)
                .orElse(null);
    }
    ...
    private byte[] createCheckText(Check check) {
